			...
			<div id="container_box">
				<section id="content">
					<ul>
						<li>
							<div class="allCheck">
								<label for="allCheck"><input type="checkbox" name="allCheck" id="allCheck" /> 모두 선택</label>
								<script>
									$("#allCheck").click(function() {
										var chk = $("#allCheck").prop("checked");
										if(chk) $(".chBox").prop("checked", true);
								...
								<script>
									$(".selectDelete_btn").click(function() {
										...
										if(confirm_val) {
											...
											$.ajax({
												url : "/shop/deleteCart.json",
												type : "post",
												data : { chbox : checkArr },
												success : function(result) {
													if (result == 1) location.href = "/shop/cartList";
													else alert('삭제 실패');
												}
											});
										}
						...
						<c:forEach items="${cartList}" var="cartList">
							<li>
								...
								<div class="thumb">
									<c:choose>
										<c:when test="${cartList.gdsImg != null}">
											<!-- 이미지 있을때 출력 -->
											<img alt="${cartList.gdsNum}번 상품 썸네일" src="/resources/imgUpload${cartList.gdsImg}">
										</c:when>
										<c:otherwise>
											<!-- 이미지 없을때 출력 -->
											<img alt="${cartList.gdsNum}번 상품 썸네일" src="/resources/imgUpload/none.png">
										</c:otherwise>
									</c:choose>
								</div>
								<div class="gdsInfo">
